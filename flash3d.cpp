  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    for (std::size_t idx = begin; idx < end; ++idx) {
      ...
      for (int m = 0; m < Mloc; ++m) {
        double acc = 0.0;
        const double *ct = &ctab[m]; // note: we must stride by M when indexing
        for (int t = 0; t < T; ++t) {
          // (t*M + m)
...
IntegerVector flash3d_supervoxels_cpp(NumericMatrix ts,     // T x Nmask (time series for masked voxels)
  ...
  auto relax_range = [&](size_t begin, size_t end, int step)->void {
    for (size_t idx = begin; idx < end; ++idx) {
      if (!is_mask[idx]) { best_next[idx] = best[idx]; label_next[idx] = label[idx]; carry_next[idx] = carry[idx]; continue; }
      float b = best[idx];
      int lab = label[idx];
      int car = carry[idx];
      ...
      // examine 26 neighbors at stride "step"
      ...
      best_next[idx] = b; label_next[idx] = lab; carry_next[idx] = car;
    }
  };
  ...
  for (int r = 0; r < rounds; ++r) {
    if (verbose) Rcpp::Rcout << "[FLASH-3D] Round " << (r+1) << " / " << rounds << " ...\n";
    ...
    for (int s = ceil_pow2(maxdim); s >= 1; s >>= 1) {
      ...
      parallelFor(0, Ngrid, [&](size_t b, size_t e){ relax_range(b,e,s); }, /*grain*/ 65536);
    }
    ...
    parallelFor(0, Ngrid, [&](size_t b, size_t e){
