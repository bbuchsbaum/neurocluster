...
- **Single-volume note**: When input is a 3D `NeuroVol`, features are
  left unscaled to preserve raw Euclidean intensities for
  superpixel-style clustering.
...
- **Single-volume note**: Feature normalization defaults to
  per-timepoint z-scoring; with one timepoint this just mean-centers
  intensities, keeping Euclidean distances intact.
...
The unified interface exposes these standardized parameters, but each
method accepts only the parameters that affect its algorithm; inactive
...
- `max_iterations`: Iteration limit for iterative methods; unsupported
  by SNIC, Slice-MSF, and commute-time clustering
...
- [neurosurf](https://github.com/bbuchsbaum/neurosurf): Surface-based
  clustering
- [neighborweights](https://github.com/bbuchsbaum/graphweights): Spatial
  weight matrices
