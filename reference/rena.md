...
- mask:
  ...
  A `NeuroVol` mask defining the voxels to include in clustering.
...
- exact_k:
  ...
  Logical; whether to use the shared adjacency-preserving merge and
  split engine to ensure exactly K clusters. A target below the number
  of disconnected mask components is infeasible.
...
- **Parallel alternatives**: In the unified API, `supervoxels` and
  `slic` support `parallel = TRUE`. The other methods do not advertise a
...
with `method = "rena"` for a standardized interface across all
clustering methods.
...
for iterative heat kernel clustering,
[`snic`](https://bbuchsbaum.github.io/neurocluster/reference/snic.md)
for non-iterative priority queue clustering
