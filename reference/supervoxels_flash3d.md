...
- lambda_s:
  ...
  Finite non-negative spatial weight (default 0.6). It is annealed
  upward over rounds.

- lambda_t:

  Finite non-negative temporal weight for Hamming distance of
  time-series hashes (default 1.0).

- lambda_g:

  Finite non-negative boundary-field weight (default 0.0). If positive,
  `barrier` is required.
...
- barrier:
  ...
  Optional non-negative finite 3D boundary field with the same geometry
  as the mask. A candidate site is penalized when its seed and the
...
with `method = "flash3d"` for a standardized interface across all
clustering methods.
