...
inline void rag_agglomerate_to_K_per_slice(
  ...
  for (int z=0; z<nz; ++z) {
    ...
    for (int y=0;y<ny;++y) for (int x=0;x<nx;++x) {
      ...
      if (A<0) continue;
      // neighbors: E,S,(diag)
      auto reg=[&](int nidx){ if (mask[nidx]){ int B=voxel2group[nidx]; if (B>=0 && A!=B){ int u=std::min(A,B), v=std::max(A,B); pairs.emplace(u,v);} } };
      if (x+1<nx) reg(idx3d(x+1,y,z,nx,ny));
      if (y+1<ny) reg(idx3d(x,  y+1,z,nx,ny));
      if (nbhd==8) {
        if (x+1<nx && y+1<ny) reg(idx3d(x+1,y+1,z,nx,ny));
        if (x+1<nx && y-1>=0) reg(idx3d(x+1,y-1,z,nx,ny));
      }
    }
    // run a local global-agglomeration limited to these groups
    // (We simply call global agglomeration and rely on UF + pairs restriction)
    rag_agglomerate_to_K_global(G, targetK, pairs, sumU, sz, uf);
  }
}
