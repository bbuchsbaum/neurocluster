...
datasets <- list(
  ...
  blobs_small = function() {
    syn <- generate_synthetic_volume(
      ...
      n_clusters = 4, n_time = 40, seed = 2
    )
    syn$truth_k <- syn$n_clusters
    syn
  },
  checker_medium = function() {
    syn <- generate_synthetic_volume(
      scenario = "checkerboard", dims = c(32, 32, 8),
      n_clusters = 6, n_time = 50, seed = 3
    )
    syn$truth_k <- syn$n_clusters
    syn
  },
  amplitude_only = function() {
    # Clusters differ only by amplitude (mean/variance), not temporal shape.
    dims <- c(20, 20, 6)
    n_time <- 50
    n_clusters <- 4
    seed <- 7
    set.seed(seed)
    base <- sin(seq(0, 2 * pi, length.out = n_time))
    # Spatial layout from gaussian blobs to stay consistent with other sets
    syn0 <- generate_synthetic_volume(
    ...
    truth <- syn0$truth
    mask <- syn0$mask
    amps <- runif(n_clusters, 0.5, 1.5)
    nvox <- length(truth)
    signals <- matrix(0, nrow = nvox, ncol = n_time)
    for (k in seq_len(n_clusters)) {
      idx <- which(truth == k)
      if (length(idx) == 0) next
      signals[idx, ] <- amps[k] * matrix(rep(base, each = length(idx)), nrow = length(idx)) +
        matrix(rnorm(length(idx) * n_time, sd = 0.05), nrow = length(idx))
    }
    vols <- lapply(seq_len(n_time), function(tt) {
      v <- array(0, dims)
      v[mask > 0] <- signals[, tt]
      NeuroVol(v, NeuroSpace(dims))
    })
    list(
...
method_grid <- list(
  ...
  supervoxels = list(
    list(param_id = 1, alpha = 0.3, iterations = 30, sigma1 = 1.0, sigma2 = 1.8, use_gradient = FALSE, connectivity = 10),
  ...
  consensus_combo = list(
    list(
      ...
      super_alpha = 0.3, super_sigma1 = 1.0, super_sigma2 = 1.8, super_connectivity = 10,
    ...
    for (p in param_list) {
      ...
      if (m == "snic") {
        ...
        args <- list(
          ...
          sigma1 = if (!is.null(p$sigma1)) p$sigma1 else NULL,
          sigma2 = p$sigma2,
      ...
      if (m == "consensus_combo") {
        ...
        if (m == "flash3d" && !inherits(res$res, "error") && nvox <= 5000) {
          res$res <- post_merge_to_k(res$res, base_k)
        }
      }
